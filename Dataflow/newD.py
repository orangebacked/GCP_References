...
#### New Price
   ...
   def new_venta(row):
       return row.cantidad_n*row.precioventa_n 
   ...
   class tests_and_replace(beam.DoFn):
       ...
       def process(self, element):
           ...
           try:
               ...
               df['was_price_changed'] = df['precioventa'].apply(change_with_rule0, args=(upper,lower))
...
#### define the variables to write to a table in BQ
   ...
   table_schema = ('CATEGORIA:STRING, CiudadM:STRING, Codigo:INTEGER, DESCRIPCION_FINAL:STRING, DPTO:STRING, EAN:INTEGER, FABRICANTE:STRING, Fechatransaccion:TIMESTAMP, MARCA:STRING, PRESENTACION:STRING, SECCION:STRING, SUBCATEGORIA:STRING, Sale_id:STRING, Tipologia_comercial:STRING, Week:INTEGER, cantidad:INTEGER, cantidad_n:FLOAT, precioventa:INTEGER, precioventa_n:FLOAT, totalventa:INTEGER, was_row_changed:FLOAT, was_set_changed:FLOAT, was_cantidad_changed:FLOAT, was_price_changed:FLOAT, precioventa_inconsistency:FLOAT')
